...
assert.equal(D.select({sort:'oldest'})[0].published,'2026-07-29');
...
assert.equal(D.select({uf:'SP'}).length,4);
assert.equal(D.select({institute:'Datafolha',uf:'SP'})[0].registration,'SP-04189/2026 · BR-03904/2026');
assert.equal(D.select({institute:'Quaest',uf:'SP'})[0].sample,1650);
assert.equal(D.select({institute:'AtlasIntel',uf:'SP'})[0].sample,1810);
assert.equal(D.select({institute:'Real Time Big Data',uf:'SP'})[0].registration,'SP-02794/2026');
